    ...
    while True:
        ...
        for event in pygame.event.get():
            ...
            if event.type == MOUSEBUTTONDOWN:
                ...
                # 스토리 모드 객체 추가
                if chapter1_rect.collidepoint(mouse_pos):
                    if STORY.Is_story_on[0] == False: # 추가
                        STORY.Is_story_on[0] = True
                        if STORY.Is_story_on[1] == True:
                            STORY.Is_story_on[1] = False
                    else:
                        STORY.Is_story_on[0] = False
                elif chapter2_rect.collidepoint(mouse_pos):
                    if STORY.Is_story_on[1] == False: # 추가
                    ...
                    if STORY.Is_story_on[2] == False: # 추가
                        STORY.Is_story_on[2] = True
                    else:
                        STORY.Is_story_on[2] = False
                elif chapter4_rect.collidepoint(mouse_pos):
                    if STORY.Is_story_on[3] == False: # 추가
                        STORY.Is_story_on[3] = True
                    else:
                        STORY.Is_story_on[3] = False 
        ...
        text_rules = text_format("SPECIAL RULES", uno.font, 40, (0,0,0)) # 추가    
        if STORY.Is_story_on[0] == False:  # 추가
            text_chapter1 = text_format("STORY 1", uno.font, 50, (0,0,0))
        else:
            text_chapter1 = text_format("STORY 1", uno.font, 50, (0,0,255))
        if STORY.Is_story_on[1] == False:  # 추가
            text_chapter2 = text_format("STORY 2", uno.font, 50, (0,0,0))
        else:
            text_chapter2 = text_format("STORY 2", uno.font, 50, (0,0,255))
        if STORY.Is_story_on[2] == False:  # 추가
            text_chapter3 = text_format("STORY 3", uno.font, 50, (0,0,0))
        else:
            text_chapter3 = text_format("STORY 3", uno.font, 50, (0,0,255))
        if STORY.Is_story_on[3] == False:  # 추가
            text_chapter4 = text_format("STORY 4", uno.font, 50, (0,0,0))
        else:
            text_chapter4 = text_format("STORY 4", uno.font, 50, (0,0,255))
        ...
        rules_rect = pygame.Rect(int(uno.screen_width*(50/1000)), int(uno.screen_height*(60/600)), 100, 50) # 추가
        chapter1_rect = pygame.Rect(int(uno.screen_width*(50/1000)), int(uno.screen_height*(120/600)), 100 , 50)
        chapter2_rect = pygame.Rect(int(uno.screen_width*(50/1000)), int(uno.screen_height*(180/600)), 100, 50)
        chapter3_rect = pygame.Rect(int(uno.screen_width*(50/1000)), int(uno.screen_height*(240/600)), 100, 50)
        chapter4_rect = pygame.Rect(int(uno.screen_width*(50/1000)), int(uno.screen_height*(300/600)), 100, 50)
        ...
        uno.screen.blit(text_rules, rules_rect) # 추가
...
def draw_achievement_success(tk_root, Achieve_system, num):
    icon_path = os.path.splitext(Achieve_system.achieve_list[num].icon)[0] # 경로에서 .png 확장자만 제거
    tk_root.iconbitmap(icon_path + ".ico") # .ico 확장자를 가진 경로로 아이콘 설정
